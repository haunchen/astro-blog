...
- **Reason**: 消除 W3C Feed Validator 缺 self link 警告。
...
- **Reason**: 原實作主要頁面僅列首頁，未達「列出今天存在的頁面」的本意，明確化頁面清單。
...
- **Reason**: X-XSS-Protection 已廢棄且舊瀏覽器有 side-channel 風險；`/images/*` 路徑不存在於建置輸出；補 cover.webp 與 n8n-resources 圖的快取。
