...
3. ~~**查證 Google 現行對 EEA/UK 流量未設 CMP 的處理方式**~~ **已完成**（2026-08-04）：政策自 2024-01-16 生效至今未變——未使用 Google 認證 CMP 時，Google 會**限制對 EEA/UK 使用者顯示的廣告**（多半只投放非個人化廣告，或依設定完全不投放），而非將帳號判為違規。D7「第一版不裝 CMP」因此仍然成立，代價僅止於歐洲流量的收益（非個人化廣告的 eCPM 約低 50-70%）。台灣讀者為主的站台可接受，有實際歐洲流量時再重新評估。
